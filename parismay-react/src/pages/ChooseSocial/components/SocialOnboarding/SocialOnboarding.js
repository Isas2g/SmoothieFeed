...
import woman from "@assets/images/onboarding-choose-social.png";
import {Link} from "react-router-dom";
import {routes} from "@configs/routes";

const SocialOnboarding = ({display}) => {
    return (
        <div className={"onboarding" + (display !== undefined ? " " + display : "")}>
            ...
            <div className="onboarding__dots">
                ...
                <Link to={routes.chooseSocial.index} className="onboarding__dot active"></Link>
                <Link to={routes.programs.index} className="onboarding__dot"></Link>
                <Link to={routes.news.index} className="onboarding__dot"></Link>
            </div>
            <Link to={routes.programs.index} className="onboarding__next">Далее</Link>
        </div>
    );
};

export default React.memo(SocialOnboarding);