...
import com.example.smoothiefeedapp.presentation.view_models.RegistrationFragmentViewModel
        ...
        with(binding) {
            viewLifecycleOwner.lifecycleScope.launch(Dispatchers.Main) {
                viewLifecycleOwner.lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                    ...
                    launch {
                        viewModel.userWasCreatedObserver.collect { userWasCreated ->
                            if (userWasCreated) {
                    ...
                    launch {
                        viewModel.exceptionsWithUserCreatingObserver.collect { exception ->
                            if (exception != null) {
