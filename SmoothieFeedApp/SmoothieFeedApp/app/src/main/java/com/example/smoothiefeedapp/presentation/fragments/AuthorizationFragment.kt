...
import com.example.smoothiefeedapp.presentation.view_models.AuthorizationFragmentViewModel
...
class AuthorizationFragment : Fragment(R.layout.fragment_authorization) {
    ...
    private val viewModel: AuthorizationFragmentViewModel  by viewModel()
            ...
            webViewClient = object : WebViewClient() {
                override fun shouldOverrideUrlLoading(view: WebView, url: String): Boolean {
                    if (url.startsWith("https://oauth.vk.com/blank"))
                        viewModel.saveVKToken(url, "vk")//add logic with another social networks
                    return false
                }
            }
        }
    ...
    private fun bindViewModel() {
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch {
                    viewModel.getVKUserUseCase().collect {
                        if (!it.isNullOrEmpty()) requireActivity().onBackPressed()
                    }
                }
            }
        }
